...
public class Add : Command.ICommand {
    ...
    public string Guard(string[] args) {
        ...
        if (!(int.TryParse(args[2], out int xPos)) ||
            !(int.TryParse(args[3], out int yPos))) {
            return "Coordinates are not valid integers.";
        }

        Active.guards.Add((xPos, yPos));

        foreach (var item in Active.guards) {
            Console.WriteLine(item);
        }

        return $"Successfully added {args[0]} obstacle.";
    }
}
